...
//  Pods
//
//  Created by vivianzzhu on 2019/7/16.
//
// Initial work from:
...
@implementation EXApplicationUID
...
+ (NSString *)uid {
  return [[[EXApplicationUID alloc] initWithKey:@"applicationUID"] uid];
}
 ...
 */
- (void)save {
  if (![EXApplicationUID valueForUserDefaultsKey:_uidKey]) {
    [EXApplicationUID setValue:_uid forUserDefaultsKey:_uidKey];
  }
  if (![EXApplicationUID valueForKeychainKey:_uidKey service:_uidKey]) {
    [EXApplicationUID setValue:_uid forKeychainKey:_uidKey inService:_uidKey];
  }
}
